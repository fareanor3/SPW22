...
#ifndef _PE_DYNAMIC_TREE_H_
#define _PE_DYNAMIC_TREE_H_
...
PE_INLINE bool PE_TreeNode_IsLeaf(PE_TreeNode *node)
{
...
typedef struct PE_DynamicTree_s
{
    PE_Id m_root;
    PE_TreeNode *m_nodes;
    int m_nodeCount;
...
PE_INLINE PE_ColliderProxy *PE_DynamicTree_GetProxy(PE_DynamicTree *tree, PE_Id proxyId)
{
    assert(0 <= proxyId && proxyId < tree->m_capacity);
    return tree->m_nodes[proxyId].proxy;
}

PE_INLINE PE_AABB *PE_DynamicTree_GetFatAABB(PE_DynamicTree *tree, PE_Id proxyId)
{
    assert(0 <= proxyId && proxyId < tree->m_capacity);
    return &(tree->m_nodes[proxyId].aabb);
}

PE_INLINE bool PE_DynamicTree_WasMoved(PE_DynamicTree *tree, PE_Id proxyId)
{
    assert(0 <= proxyId && proxyId < tree->m_capacity);
    return tree->m_nodes[proxyId].moved;
}

PE_INLINE void PE_DynamicTree_ClearMoved(PE_DynamicTree *tree, PE_Id proxyId)
{
    assert(0 <= proxyId && proxyId < tree->m_capacity);
    tree->m_nodes[proxyId].moved = false;
}

PE_INLINE int PE_DynamicTree_GetHeight(PE_DynamicTree *tree)
{
...
typedef struct PE_TreeQueryOutput_s
{
    PE_Id *ids;
    int idCount;
} PE_TreeQueryOutput;

PE_INLINE void PE_DynamicTree_Query(
    PE_DynamicTree *tree, PE_AABB *aabb, PE_TreeQueryOutput *output)
{
    ...
    while (PE_IdStack_GetSize(stack) > 0)
    {
        ...
        if (PE_AABB_Overlap(&node->aabb, aabb))
        {
